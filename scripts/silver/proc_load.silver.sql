...
BEGIN
	...
	BEGIN TRY 
		...
		PRINT'the loading time is' + CAST(datediff(second, @start_time, @end_time) AS NVARCHAR) + ' seconds'
		PRINT'>> ----------------------------------------'
		---------------------------------------------------------------------------------------
		SET @start_time = GETDATE();
		...
		prd_nm, 
		ISNULL(prd_cost,0) AS prd_cost, --- if buisness allows it
		...
		PRINT'the silver total loading time is' + CAST(datediff(second, @batch_start, @batch_end) AS NVARCHAR) + ' seconds'
	END TRY
	BEGIN CATCH 
		PRINT'============================================';
